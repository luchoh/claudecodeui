# PRD: CloudCLI Security Hardening (v3)
...
- **Changes from v2**: Aligns with current code paths (WebSocket route, SSE auth), clarifies platform mode scope, expands command execution audit, and fixes severity totals

---

## Executive Summary

CloudCLI (Claude Code UI) is a web-based interface for Claude Code, Cursor CLI, and Codex that enables remote access from mobile devices. A comprehensive security audit identified **2 critical**, **6 high**, **6 medium**, and **1 low** severity vulnerabilities that must be addressed before production deployment.
...
- Platform mode (IS_PLATFORM, set via VITE_IS_PLATFORM) bypasses JWT auth; this PRD assumes VITE_IS_PLATFORM is false for self-hosted/VPN use
...
**v3 Decision: Implement ticket-based authentication + remove long-lived URL tokens**

**Requirements:**
1. New endpoint `POST /api/ws-ticket` returns single-use ticket
2. Ticket valid for 30 seconds, stored in memory/DB
3. WebSocket connects with ticket: `ws://host/shell?ticket=abc123`
4. Server validates ticket, deletes it, establishes session
5. Remove support for JWTs in WebSocket URL query parameter
6. SSE endpoints must not accept long-lived JWTs in URL query parameters (use short-lived SSE ticket or a header-capable transport)
...
ws://host/shell?ticket=abc123def456
...
- [ ] WebSocket no longer accepts JWTs via URL query
- [ ] SSE endpoints do not accept long-lived JWTs in URL query parameters
...
**ID**: SEC-006
**Severity**: HIGH
**Location**: `server/index.js:1054-1091` + command execution sites in `server/routes/*` (see File Changes Summary)
...
3. Update `validateWorkspacePath` to support comma-separated WORKSPACES_ROOT values and validate against each resolved root
4. Use `spawn()` with `cwd` option instead of `cd` in shell
5. Pass command arguments as arrays, never interpolate into shell strings
6. Reject paths containing `..` after normalization
7. Audit all command execution sites and document any user-influenced arguments (see File Changes Summary for current locations)
...
- [ ] `validateWorkspacePath` supports multiple comma-separated roots
...
- [ ] Command execution sites audited and documented; user-supplied args are allowlisted or parameterized
...
5. Keep auth tokens in Authorization header in this phase; if cookies are introduced later they must be Secure/HttpOnly/SameSite

**Acceptance Criteria:**
- [ ] Documentation includes Caddy reverse proxy configuration
- [ ] Server detects when behind HTTPS proxy
- [ ] Auth continues to use Authorization header (no auth cookies required)
- [ ] If auth cookies are introduced, they are Secure + HttpOnly + SameSite
...
| File | Changes |
|------|---------|
| `server/middleware/auth.js` | SEC-001, SEC-002, SEC-005 (SSE token handling) |
| `server/index.js` | SEC-003, SEC-004, SEC-005, SEC-006, SEC-008, SEC-012 |
| `server/routes/ws-ticket.js` | NEW: SEC-005 |
| `server/routes/projects.js` | SEC-006 (workspace root validation) |
| `server/routes/agent.js` | SEC-006 (command execution audit) |
| `server/routes/taskmaster.js` | SEC-006 (command execution audit) |
...
- [ ] SEC-005: Ticket-based WebSocket auth + SSE token handling
...
2. **WebSocket Auth Tests**
   ...
   - Verify SSE endpoints do not accept long-lived JWTs in URL query parameters
...
## Appendix C: v3 Decision Summary

| Item | v2 Decision | v3 Update | Rationale |
|------|-------------|-----------|-----------|
| SEC-005 | Ticket-based WebSocket auth | Align WS path to `/shell` and remove long-lived URL tokens for WebSocket + SSE | Matches current server routing and avoids JWTs in query params |
| SEC-006 | Allowlist + parameterized execution | Expand scope to all command execution sites; require multi-root WORKSPACES_ROOT support | Reduces injection risk beyond the PTY flow |
| SEC-013 | Low → Medium (upgraded) | Set severity to Medium consistently | Aligns severity counts with item list |
